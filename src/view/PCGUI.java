...
//                String proElement = proName + dots + Integer.toString(n + 1);
            ...
            case "subtract one":
                int conInd;
                if (listOfConSelect.isSelectionEmpty()) {
                    conInd = listOfProSelect.getSelectedIndex();
                    prosString.get(conInd).addSubWeight(false);
                    this.ArrayIntoListModel(prosString, dataListOfPros);
                } else {
                    conInd = listOfConSelect.getSelectedIndex();
                    consString.get(conInd).addSubWeight(false);
                    this.ArrayIntoListModel(consString, dataListOfCons);
                }
                break;
        }
